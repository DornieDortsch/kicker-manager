...
describe('Table', () => {
    ...
    test('Table with Transfer History HTML Page', () => {
        // Given
        const html = TestFile.loadHTML('./lib/scraper/TableTransferHistory.test.html')

        const spielerSpy = jest.fn()
        
        // When
        Table(html, 'spieler', 8, spielerSpy)

        // Then
        expect(spielerSpy.mock.calls.length).toBe(3)

        const columns = spielerSpy.mock.calls[0][0]
        expect(columns.isSeperator).toEqual(false)
        
        const seperatorColumns = spielerSpy.mock.calls[1][0]
        expect(seperatorColumns.isSeperator).toEqual(true)
    })
})