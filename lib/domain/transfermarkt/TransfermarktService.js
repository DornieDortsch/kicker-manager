...
export default (client, repository, cache, master = false) => {
    ...
    const repositoryFile = 'trasnfermark.json'
    ...
    const init = async () => {
        ...
        try {
            repositoryTransfermarkt = await repository.readCollection()[repositoryFile]
        } catch(e) {
            console.error('read repository collection', e)
        }
    ...
    const saveState = (currentPlayer, delta, date) => {
        ...
        if(master) {
            repository.writeCollection(fileName, transfermarkt)
                .then(() => console.log('succesfully write repository file ' + fileName))
                .catch(error => console.error('error write repository file ' + fileName, error))
        }
